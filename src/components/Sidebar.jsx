 ...
 * Displays the main navigation links, the application logo, and the user profile menu.
 * Adapts layout between mobile (bottom/top bar) and desktop (left vertical bar).
 */
const Sidebar = () => {
  ...
  // Local state to toggle the logout dropdown menu
  const [showLogout, setShowLogout] = React.useState(false);

  /**
   * Handles user logout process
   * Clears Redux state, localStorage, and redirects to login
   */
  const handleLogout = () => {
    dispatch(logout());
    navigate("/login");
  ...
  return (
    <aside className="fixed left-0 top-0 w-full h-20 flex flex-row items-center px-4 bg-semi-dark-blue md:left-4 md:top-4 md:h-[calc(100vh-32px)] md:w-24 md:flex-col md:py-8 md:rounded-2xl z-50">
      ...
      <div className="md:mt-auto ml-auto md:ml-0 flex items-center md:flex-col gap-4 relative">
        ...
        {/* Avatar Image (Click to toggle logout menu) */}
        <div
